...
function NotesCard({ note, openFormEntryData, completed, important, selectedNote }) {
  ...
  return (
    <li className={`notesList-container__card ${completed ? 'completed' : ''} ${note.id === selectedNote.id ? 'selected' : ''}`} key={note.id}  onClick={openFormEntryData}>
